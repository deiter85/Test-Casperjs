...
casper.test.begin('Comprobar pagina ASPgems UK - Test45 - Services-Keep Going', 7, function suite(test) {    
  	casper.start();
  	casper.thenOpen('/en', function() {
   	  	casper.click("a[href='/en/services']"); //hacer click en servicio
    });
    casper.then(function() {
   	  	casper.click("a[href='/en/services/keep-going']"); //hacer click en keep going
    });
  	casper.then(function() {		
  	   	test.assertUrlMatch('https://aspgems.com/en/services/keep-going', 'Direccion URL correcta'); //comprobar la direccion URL
    });
    casper.then(function() {
  		  test.assertExists('h1', 'Se encuentra el titulo'); //comprobar si se encuentra el titulo
    });
    casper.then(function() {
        test.assertSelectorHasText('h1', 'Keep going', 'El texto del titulo es el correcto'); //comprobar si el titulo es el correcto
    });
    casper.then(function() {
