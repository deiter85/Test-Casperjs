...
casper.test.begin('Comprobar pagina ASPgems UK - Test43 - Services-Technical Management', 7, function suite(test) {    
  	casper.start();
  	casper.thenOpen('/en', function() {
   	  	casper.click("a[href='/en/services']"); //hacer click en servicio
    });
    casper.then(function() {
   	  	casper.click("a[href='/en/services/technical-management']"); //hacer click en direccion tecnica
    });
  	casper.then(function() {		
  	   	test.assertUrlMatch('https://aspgems.com/en/services/technical-management', 'Direccion URL correcta'); //comprobar la direccion URL
    });
    casper.then(function() {
  		  test.assertExists('h1', 'Se encuentra el titulo'); //comprobar si se encuentra el titulo
    });
    casper.then(function() {
        test.assertSelectorHasText('h1', 'Technical Management', 'El texto del titulo es el correcto'); //comprobar si el titulo es el correcto
    });
    casper.then(function() {
